...
if __name__ == "__main__":
    ...
    ed = datetime(2022, 4, 5)
    df = yf.download(tickers='BABA', start=sd, end=ed, interval="1d")
    df = df[['Open', 'High', 'Low', 'Close', 'Volume']]

    fig = volume_profile(df, price_pace=5, return_raw=False)
    fig.show()

    # fig = market_profile(df, price_pace=0.75, time_pace='30T', return_raw=False)
    # fig.show()
